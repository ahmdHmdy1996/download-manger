...
class DownloadManagerUI {
  ...
  init() {
    ...
    this.browsePathBtn.addEventListener("click", () =>
    ...
    ipcRenderer.on("download-added", (event, download) => {
  ...
  async loadDownloads() {
    ...
    downloads.forEach((download) => {
      console.log(
        `Loading download: ${download.id} - ${download.filename} (${download.status})`
      );
      this.addDownloadToUI(download);
    });
  ...
  async addDownload() {
    ...
    if (result.success) {
      ...
      Utils.showNotification(
        `Failed to add download: ${result.error}`,
        "error"
      );
    }
  }
  ...
  addDownloadToUI(download) {
    ...
    this.downloadsList.insertBefore(
      downloadItem,
      this.downloadsList.firstChild
    );
  ...
  attachDownloadActions(downloadId) {
    ...
    console.log(
      `Attaching ${actionButtons.length} action buttons for download ${downloadId}`
    );
  ...
  updateDownloadProgress(downloadId, progress) {
    ...
    if (!download) {
      // Fallback: If we receive progress for a download we don't know about,
      // allow a quick refresh to fetch it from the main process.
      // This handles cases where 'download-added' event might have been missed
  ...
  updateDownloadStatus(downloadId, status, error = null) {
    ...
    if (status === "completed") {
      Utils.showNotification(
        `Download completed: ${download.filename}`,
        "success"
      );
  ...
  updateEmptyState() {
    const visibleItems = Array.from(
      document.querySelectorAll(".download-item:not(.hidden)")
    );
  ...
  async openSettings() {
    ...
    document.getElementById("notificationsEnabledInput").checked =
      notificationSettings.enabled;
    document.getElementById("notificationSoundInput").checked =
      notificationSettings.sound;
    document.getElementById("notifyOnCompleteInput").checked =
      notificationSettings.onComplete;
    document.getElementById("notifyOnErrorInput").checked =
      notificationSettings.onError;
    document.getElementById("notifyOnPauseInput").checked =
      notificationSettings.onPause;
    document.getElementById("notifyOnResumeInput").checked =
      notificationSettings.onResume;

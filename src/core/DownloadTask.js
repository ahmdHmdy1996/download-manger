...
class DownloadTask extends EventEmitter {
  ...
  async start() {
    ...
    try {
      ...
      if (this.totalSize === 0) {
        ...
        console.log(`[DownloadTask] File info fetched. Filename: ${this.filename}, Size: ${this.totalSize}`);
      }

      // Decide download method
      if (this.supportsRanges && this.totalSize > 1024 * 1024) { // > 1MB
  ...
  handleError(error) {
    ...
    if (error.name === 'AbortError' || error.message === 'canceled' || error.message === 'aborted') return;
  ...
  async singleThreadedDownload() {
    try {
      ...
      console.log(`Starting download: ${this.url}, Resume: ${isResume}, StartByte: ${startByte}`);
    ...
    response.data.on('data', (chunk) => {
      ...
      this.speed = this.calculateSpeed(loadedSinceStart, Date.now() - startTime);
  ...
  async multiThreadedDownload() {
    ...
    // Download chunks
    await Promise.all(this.chunks.map(chunk => this.downloadChunk(chunk, tempDir)));
  ...
  async downloadChunk(chunk, tempDir) {
    ...
    while (attempt < maxRetries) {
      try {
        if (this.status === 'paused' || this.status === 'cancelled' || this.status === 'error') return;
        ...
        const config = NetworkUtils.getAxiosConfig(this.url, {
          ...
          responseType: 'stream',
          signal: this.abortController.signal,
          headers: { Range: `bytes=${currentStart}-${chunk.end}` },
          timeout: 60000
        });
        ...
        attempt++;
        console.warn(`Chunk ${chunk.index} failed (Attempt ${attempt}/${maxRetries}): ${error.message}`);
  ...
  updateGlobalProgress() {
    this.downloadedSize = this.chunks.reduce((acc, chunk) => acc + chunk.downloaded, 0);
    ...
    if (timeDiff >= 1000) { // Update speed every second
      ...
      if (this.speed > 0) {
        ...
        console.log(`[ETA Debug] Speed: ${this.speed}, Remaining: ${remainingBytes}, ETA: ${this.eta}`);
      } else {
        this.eta = 0;

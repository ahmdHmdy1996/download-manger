...
class NetworkUtils {
  ...
  static async fetchFileInfo(url, options = {}) {
    // Ensure URL is clean
    ...
    for (let i = 0; i < retries; i++) {
      try {
        ...
        if (
          error.response &&
          (error.response.status === 405 || error.response.status === 403)
        ) {
    ...
    try {
      const config = this.getAxiosConfig(cleanUrl, {
        method: "GET",
        headers: { Range: "bytes=0-0", ...customHeaders },
        ...options,
      });
      const response = await axios(config);
      return this.processResponse(response, cleanUrl);
  ...
  static processResponse(response, url) {
    ...
    const supportsRanges =
      ...
      (response.headers["content-range"] &&
        response.headers["content-range"].includes("bytes"));
